...
class EKPabe(ABEnc):
    ...
    def setup(self, attributes, seed):
        s = group.random(ZR,seed=seed)
        g = group.random(G1,seed=seed)
        
        self.attributeSecrets = {}
        self.attribute = {}
        for i,attr in enumerate(attributes):
            si = group.random(ZR,seed=seed+1+i)
...
def initialize(attributes,seed):
    groupObj = PairingGroup('MNT224')
    kpabe = EKPabe(groupObj)

    (pk, mk) = kpabe.setup(attributes,seed) #get from global_variables

    return groupObj, kpabe, pk, mk
    # # policy = '(ONE or THREE) and (THREE or TWO)'
